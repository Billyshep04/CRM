...
class ProcessWebsiteProvisioning implements ShouldQueue { use Dispatchable,InteractsWithQueue,Queueable,SerializesModels; public int $tries=1; public int $timeout=600; public function __construct(public int $runId){} public function handle(WebsiteProvisioner $provisioner):void{if($run=WebsiteProvisioningRun::find($this->runId))$provisioner->process($run);} }
