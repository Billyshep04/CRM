...
class WebsiteManagementController extends Controller
{
    ...
    public function delete(Request $request,Website $website,WebsiteDeletionService $service){$d=$request->validate(['deletion_type'=>['required',Rule::in(['crm_only','hosting_and_crm'])],'confirmation'=>['required','string','max:255'],'backup_confirmed'=>['exclude_unless:deletion_type,hosting_and_crm','required','accepted'],'other_domains_confirmed'=>['sometimes','boolean'],'idempotency_key'=>['required','uuid']]);try{$audit=$service->delete($website,$d['deletion_type'],$d['confirmation'],$d['idempotency_key'],$request->user()->id,(bool)($d['backup_confirmed']??false),(bool)($d['other_domains_confirmed']??false));return response()->json(['data'=>$audit,'message'=>'Website deletion completed.']);}catch(RuntimeException $e){return response()->json(['message'=>$e->getMessage()],422);}}
