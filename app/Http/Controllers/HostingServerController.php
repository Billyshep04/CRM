...
class HostingServerController extends Controller
{
    ...
    public function update(Request $request, HostingServer $hostingServer) { $data=$this->validated($request, true); if(isset($data['credentials'])&&empty($data['credentials']['token']))$data['credentials']['token']=$hostingServer->credentials['token']??null; if(isset($data['metadata']))$data['metadata']=[...($hostingServer->metadata??[]),...$data['metadata']]; $hostingServer->update($data); return response()->json(['data' => $hostingServer->fresh()]); }
    ...
    private function validated(Request $request, bool $sometimes = false): array
    {
        ...
        return $request->validate(['name' => [$required, 'string', 'max:255'], 'provider' => ['sometimes', 'string', 'max:50'], 'hostname' => ['nullable', 'string', 'max:255'], 'server_type' => ['nullable', 'string', 'max:100'], 'api_type' => ['sometimes', Rule::in(['mock', 'cpanel', 'whm'])], 'credentials' => ['sometimes', 'nullable', 'array'], 'credentials.username' => ['nullable', 'string'], 'credentials.token' => ['nullable', 'string'], 'status' => ['sometimes', Rule::in(['active', 'paused'])], 'metadata' => ['sometimes', 'nullable', 'array'], 'metadata.ssh_host_fingerprint' => ['nullable', 'string', 'max:255', 'regex:/^(?:sha256:)?[a-z0-9+\/:]{32,}={0,2}$/i'], 'metadata.development_base_domain' => ['nullable', 'string', 'max:253', 'regex:/^(?!-)(?:[a-z0-9-]{1,63}\.)+[a-z]{2,63}$/i']]);
    }
}
