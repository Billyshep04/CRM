...
class WebsiteProvisioningController extends Controller {
 public function options(){return response()->json(['data'=>['mode'=>config('hosting.provisioning_mode','mock'),'live_enabled'=>config('hosting.allow_live_provisioning',false),'servers'=>HostingServer::with(['packages'=>fn($q)=>$q->where('active',true)])->where('status','active')->get(),'profiles'=>WordpressProfile::where('active',true)->get()]]);}
 public function store(Request $request){
  ...
  $run=DB::transaction(function()use($d,$request){
   ...
   foreach(['create_hosting','verify_hosting',...($d['website_type']==='wordpress'?['install_wordpress','apply_profile','connect_agent']:[]),'initial_check']as$step)$run->steps()->create(['step'=>$step]);return$run;
  });
 ...
 public function retry(WebsiteProvisioningRun $websiteProvisioningRun){if($websiteProvisioningRun->state!=='failed')throw ValidationException::withMessages(['state'=>['Only failed provisioning runs can be retried.']]);ProcessWebsiteProvisioning::dispatch($websiteProvisioningRun->id);return response()->json(['data'=>$websiteProvisioningRun->fresh(['steps','account'])],202);}
}
