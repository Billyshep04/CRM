...
class WebsiteDevelopmentWorkflowTest extends TestCase
{
    ...
    public function test_go_live_reuses_account_migrates_wordpress_and_preserves_development_domain(): void
    {
        $admin = $this->user('admin');
        ...
        Http::fake(function ($request) use (&$renamed) {
            if (str_contains($request->url(), '/modifyacct')) { $renamed = true; return Http::response(['metadata' => ['result' => 1, 'reason' => 'OK'], 'data' => []]); }
            if (str_contains($request->url(), '/listaccts')) return Http::response(['metadata' => ['result' => 1], 'data' => ['acct' => [['user' => 'devusr', 'domain' => $renamed ? 'site.test' : 'site-ab12.dev.web-stamp.co.uk', 'ip' => '192.0.2.10', 'plan' => 'Standard', 'suspended' => 0]]]]);
            if (str_contains($request->url(), '/cpanel')) return Http::response(['metadata' => ['result' => 1], 'data' => ['result' => ['data' => ['main_domain' => 'site-ab12.dev.web-stamp.co.uk', 'addon_domains' => []]]]]);
        ...
        $this->assertSame(1, $run->steps()->where('step', 'change_primary_domain')->value('attempts'));
    ...
    public function test_go_live_request_creates_a_resumable_run_without_changing_the_current_domain(): void
    {
        ...
        $this->assertDatabaseCount('website_launch_steps', 8);
    ...
    public function test_disposable_development_site_completes_the_full_go_live_lifecycle_safely(): void
    {
        ...
        $whm = (object) ['created' => false, 'domain' => null, 'username' => 'disposable1', 'modify_calls' => 0, 'modify_parameters' => [], 'fail' => false, 'create_parameters' => []];
        ...
        Http::fake(function ($request) use ($whm) {
            ...
            if (str_contains($url, '/listaccts')) return Http::response(['metadata' => ['result' => 1], 'data' => ['acct' => $whm->created ? [['user' => $whm->username, 'domain' => $whm->domain, 'ip' => '192.0.2.44', 'plan' => 'Standard', 'suspended' => 0, 'owner' => 'reseller']] : []]]);
            if (str_contains($url, '/createacct')) { $whm->created = true; $whm->domain = strtolower((string) $request['domain']); $whm->create_parameters = $request->data(); return Http::response(['metadata' => ['result' => 1, 'reason' => 'Account created'], 'data' => ['ip' => '192.0.2.44']]); }
            if (str_contains($url, '/modifyacct')) { $whm->modify_calls++; $whm->modify_parameters = $request->data(); $whm->domain = strtolower((string) $request['domain']); return Http::response(['metadata' => ['result' => 1, 'reason' => 'OK'], 'data' => []]); }
            if (str_contains($url, '/accountsummary')) return Http::response(['metadata' => ['result' => 1], 'data' => ['acct' => [['user' => $whm->username, 'shell' => '/bin/bash']]]]);
            if (str_contains($url, '/cpanel')) return Http::response(['metadata' => ['result' => 1], 'data' => ['result' => ['data' => ['main_domain' => $whm->domain, 'addon_domains' => []]]]]);
        ...
        $this->assertSame(1, $whm->modify_calls);
        $this->assertSame($whm->username, $whm->modify_parameters['user']);
        $this->assertSame($productionDomain, $whm->modify_parameters['domain']);
        $this->assertSame($productionDomain, $whm->domain);
