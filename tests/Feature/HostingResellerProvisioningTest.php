...
class HostingResellerProvisioningTest extends TestCase {
 ...
 public function test_complete_live_wordpress_pipeline_reaches_every_stage_without_false_success():void
 {
  ...
  $server=HostingServer::create(['name'=>'Krystal','api_type'=>'whm','hostname'=>'whm.example.test','credentials'=>['username'=>'reseller','token'=>'secret'],'metadata'=>['ssh_host_fingerprint'=>str_repeat('a',64)]]);
  $package=HostingPackage::create(['hosting_server_id'=>$server->id,'external_id'=>'Standard','name'=>'Standard','shell_access'=>true]);
  $response=$this->actingAs($admin)->postJson('/api/website-provisioning',$this->payload($customer,$server,$package,'pipeline.test'))->assertCreated()->assertJsonPath('data.wordpress_login.reveal_available',true);
  $run=\App\Models\WebsiteProvisioningRun::firstOrFail();
 ...
 public function test_generated_wordpress_credentials_are_encrypted_hidden_and_revealed_once():void{$admin=$this->user('admin');$customer=$this->customer();[$server,$package]=$this->hosting();$response=$this->actingAs($admin)->postJson('/api/website-provisioning',$this->payload($customer,$server,$package,'credentials.test'))->assertCreated()->assertJsonPath('data.wordpress_login.reveal_available',true);$websiteId=$response->json('data.website.id');$this->assertStringNotContainsString('password',strtolower($response->getContent()));$credential=\App\Models\WebsiteCredential::firstOrFail();$this->assertNotSame($credential->secret_encrypted,$credential->getRawOriginal('secret_encrypted'));$this->actingAs($admin)->getJson("/api/websites/{$websiteId}")->assertOk()->assertJsonPath('data.wordpress_login.reveal_available',true);$this->assertNull($credential->fresh()->revealed_at);$this->actingAs($admin)->getJson("/api/websites/{$websiteId}")->assertOk()->assertJsonPath('data.wordpress_login.reveal_available',true);$this->assertNull($credential->fresh()->revealed_at);$this->actingAs($admin)->postJson("/api/websites/{$websiteId}/reveal-credential")->assertOk()->assertJsonPath('data.username','webstamp_admin');$this->actingAs($admin)->postJson("/api/websites/{$websiteId}/reveal-credential")->assertStatus(410);$this->actingAs($admin)->getJson("/api/websites/{$websiteId}")->assertOk()->assertJsonPath('data.wordpress_login.reveal_available',false)->assertJsonPath('data.wordpress_login.revealed',true);}
 public function test_completed_development_provisioning_surfaces_stored_login_without_consuming_it():void
 {
  config(['hosting.development_base_domain'=>'dev.web-stamp.co.uk']);$admin=$this->user('admin');$customer=$this->customer();[$server,$package]=$this->hosting();$server->update(['metadata'=>['development_base_domain'=>'dev.web-stamp.co.uk']]);$payload=[...$this->payload($customer,$server,$package,'credentials-ab12.dev.web-stamp.co.uk'),'environment'=>'development'];
  $response=$this->actingAs($admin)->postJson('/api/website-provisioning',$payload)->assertCreated()->assertJsonPath('data.state','complete')->assertJsonPath('data.wordpress_login.reveal_available',true)->assertJsonPath('data.wordpress_login.reset_available',true);
  $websiteId=$response->json('data.website.id');$credential=\App\Models\WebsiteCredential::where('website_id',$websiteId)->firstOrFail();$this->assertNull($credential->revealed_at);$this->assertNotSame($credential->secret_encrypted,$credential->getRawOriginal('secret_encrypted'));
  $this->actingAs($admin)->getJson("/api/websites/{$websiteId}")->assertOk()->assertJsonPath('data.environment','development')->assertJsonPath('data.wordpress_login.reveal_available',true)->assertJsonMissingPath('data.wordpress_login.password');$this->assertNull($credential->fresh()->revealed_at);
 }
 public function test_missing_wordpress_credential_has_safe_recovery_state_and_does_not_reset_without_automation_access():void
 {
  $admin=$this->user('admin');$customer=$this->customer();$server=$this->whmServer();$account=HostingAccount::create(['hosting_server_id'=>$server->id,'external_id'=>'missingcred','username'=>'missingcred','primary_domain'=>'missing.test','status'=>'active','last_synced_at'=>now()]);$website=\App\Models\Website::create(['customer_id'=>$customer->id,'hosting_server_id'=>$server->id,'hosting_account_id'=>$account->id,'name'=>'Missing credential','domain'=>'missing.test','login_url'=>'https://missing.test/wp-admin/','environment'=>'production','wordpress_enabled'=>true,'hosting_enabled'=>true]);
  $this->actingAs($admin)->getJson("/api/websites/{$website->id}")->assertOk()->assertJsonPath('data.wordpress_login.reveal_available',false)->assertJsonPath('data.wordpress_login.reset_available',false);
  $this->actingAs($admin)->postJson("/api/websites/{$website->id}/reset-wordpress-login")->assertStatus(422)->assertJsonPath('message','Automation access is unavailable. Reset this WordPress login manually in cPanel or WordPress.');$this->assertDatabaseCount('website_credentials',0);
 }
 public function test_development_login_can_be_safely_reset_when_stored_credentials_are_unavailable():void
 {
  $runner=new class implements \App\Contracts\SshCommandRunner { public array $commands=[]; public function run(HostingServer $server,HostingAccount $account,string $password,string $command,int $timeout=60):array{$this->commands[]=$command;return['exit_code'=>0,'stdout'=>'Success','stderr'=>''];} };$this->app->instance(\App\Contracts\SshCommandRunner::class,$runner);
  $admin=$this->user('admin');$customer=$this->customer();$server=$this->whmServer();$account=HostingAccount::create(['hosting_server_id'=>$server->id,'external_id'=>'recoverdev','username'=>'recoverdev','primary_domain'=>'recover.dev.test','status'=>'active','last_synced_at'=>now(),'automation_password_encrypted'=>'cpanel-private']);$website=\App\Models\Website::create(['customer_id'=>$customer->id,'hosting_server_id'=>$server->id,'hosting_account_id'=>$account->id,'name'=>'Recover development login','domain'=>'recover.dev.test','login_url'=>'https://recover.dev.test/wp-admin/','environment'=>'development','wordpress_enabled'=>true,'hosting_enabled'=>true]);\App\Models\WebsiteProvisioningRun::create(['public_id'=>(string)\Illuminate\Support\Str::uuid(),'website_id'=>$website->id,'hosting_server_id'=>$server->id,'hosting_account_id'=>$account->id,'initiated_by_user_id'=>$admin->id,'idempotency_key'=>(string)\Illuminate\Support\Str::uuid(),'domain'=>$website->domain,'mode'=>'live','website_type'=>'wordpress','state'=>'complete','options'=>['admin_username'=>'webstamp_admin']]);
  $response=$this->actingAs($admin)->postJson("/api/websites/{$website->id}/reset-wordpress-login")->assertOk()->assertJsonPath('data.reveal_available',true);$this->assertStringNotContainsString('password',strtolower($response->getContent()));$credential=\App\Models\WebsiteCredential::firstOrFail();$this->assertSame('webstamp_admin',$credential->username);$this->assertNull($credential->revealed_at);$this->assertNotSame($credential->secret_encrypted,$credential->getRawOriginal('secret_encrypted'));$this->assertTrue(collect($runner->commands)->contains(fn($command)=>str_contains($command,"'user' 'update' 'webstamp_admin'")&&str_contains($command,"'--user_pass=")));
  $this->actingAs($admin)->getJson("/api/websites/{$website->id}")->assertOk()->assertJsonPath('data.wordpress_login.reveal_available',true);
 }
 ...
 public function test_reserved_generated_username_is_replaced_and_authoritative_username_is_persisted():void
 {
  config(['hosting.provisioning_mode'=>'live','hosting.allow_live_provisioning'=>true]);
  $createdUsername=null;$requested=[];
  Http::fake(function($request)use(&$createdUsername,&$requested){if(str_contains($request->url(),'/listaccts'))return Http::response(['metadata'=>['result'=>1,'reason'=>'OK'],'data'=>['acct'=>$createdUsername?[['user'=>$createdUsername,'domain'=>'reserved-retry.test','ip'=>'1.2.3.4','plan'=>'Standard','suspended'=>0]]:[]]]);if(str_contains($request->url(),'/createacct')){$requested[]=$request['username'];if(count($requested)===1)return Http::response(['metadata'=>['result'=>0,'reason'=>'(XID safe) “testcmmbxls” is a reserved username on this system.']]);$createdUsername=$request['username'];return Http::response(['metadata'=>['result'=>1,'reason'=>'Account created']]);}return Http::response([],404);});
  [$run,$website]=$this->singleStepLiveRun('reserved-retry.test');
  $run->update(['secrets_encrypted'=>['cpanel_password'=>'private-password','database_password'=>'database-password','wordpress_password'=>'wordpress-password','cpanel_username_proposal'=>'testcmmbxls']]);
  app(\App\Services\Hosting\WebsiteProvisioner::class)->process($run);
  $this->assertCount(2,$requested);$this->assertSame('testcmmbxls',$requested[0]);$this->assertNotSame($requested[0],$requested[1]);$this->assertMatchesRegularExpression('/^ws[a-z0-9]{10}$/',$requested[1]);
  $this->assertSame('complete',$run->fresh()->state);$this->assertSame($requested[1],$website->fresh()->cpanel_username);$this->assertSame($requested[1],$run->fresh()->account->username);
 }
 public function test_existing_username_rejection_uses_a_different_candidate():void
 {
  config(['hosting.provisioning_mode'=>'live','hosting.allow_live_provisioning'=>true]);
  $createdUsername=null;$requested=[];
  Http::fake(function($request)use(&$createdUsername,&$requested){if(str_contains($request->url(),'/listaccts'))return Http::response(['metadata'=>['result'=>1,'reason'=>'OK'],'data'=>['acct'=>$createdUsername?[['user'=>$createdUsername,'domain'=>'username-in-use.test','ip'=>'1.2.3.4','plan'=>'Standard','suspended'=>0]]:[]]]);if(str_contains($request->url(),'/createacct')){$requested[]=$request['username'];if(count($requested)===1)return Http::response(['metadata'=>['result'=>0,'reason'=>'A user with the username “existingusr” already exists.']]);$createdUsername=$request['username'];return Http::response(['metadata'=>['result'=>1,'reason'=>'Account created']]);}return Http::response([],404);});
  [$run]=$this->singleStepLiveRun('username-in-use.test');$run->update(['secrets_encrypted'=>['cpanel_password'=>'private-password','cpanel_username_proposal'=>'existingusr']]);
  app(\App\Services\Hosting\WebsiteProvisioner::class)->process($run);
  $this->assertSame('complete',$run->fresh()->state);$this->assertCount(2,array_unique($requested));$this->assertSame($requested[1],$run->fresh()->account->username);
 }
 public function test_unrelated_createacct_failure_is_not_retried_as_a_username_problem():void
 {
  config(['hosting.provisioning_mode'=>'live','hosting.allow_live_provisioning'=>true]);$createCalls=0;
  Http::fake(function($request)use(&$createCalls){if(str_contains($request->url(),'/listaccts'))return Http::response(['metadata'=>['result'=>1,'reason'=>'OK'],'data'=>['acct'=>[]]]);if(str_contains($request->url(),'/createacct')){$createCalls++;return Http::response(['metadata'=>['result'=>0,'reason'=>'The selected hosting package does not exist.']]);}return Http::response([],404);});
  [$run]=$this->singleStepLiveRun('package-failure.test');app(\App\Services\Hosting\WebsiteProvisioner::class)->process($run);
  $this->assertSame(1,$createCalls);$this->assertSame('failed',$run->fresh()->state);$this->assertSame('create_cpanel_account',$run->fresh()->failed_step);$this->assertStringContainsString('package',$run->fresh()->safe_error);
 }
 public function test_username_retries_are_bounded_and_failed_run_can_recover():void
 {
  config(['hosting.provisioning_mode'=>'live','hosting.allow_live_provisioning'=>true]);$requested=[];$rejectAll=true;$createdUsername=null;
  Http::fake(function($request)use(&$requested,&$rejectAll,&$createdUsername){if(str_contains($request->url(),'/listaccts'))return Http::response(['metadata'=>['result'=>1,'reason'=>'OK'],'data'=>['acct'=>$createdUsername?[['user'=>$createdUsername,'domain'=>'bounded-retry.test','ip'=>'1.2.3.4','plan'=>'Standard','suspended'=>0]]:[]]]);if(str_contains($request->url(),'/createacct')){$requested[]=$request['username'];if($rejectAll)return Http::response(['metadata'=>['result'=>0,'reason'=>'The username is unavailable.']]);$createdUsername=$request['username'];return Http::response(['metadata'=>['result'=>1,'reason'=>'Account created']]);}return Http::response([],404);});
  [$run,$website]=$this->singleStepLiveRun('bounded-retry.test');app(\App\Services\Hosting\WebsiteProvisioner::class)->process($run);
  $this->assertSame('failed',$run->fresh()->state);$this->assertCount(4,$requested);$this->assertCount(4,array_unique($requested));$this->assertCount(4,$run->fresh()->secrets_encrypted['cpanel_username_rejected']);$this->assertNull($website->fresh()->hosting_account_id);
  $rejectedRequests=$requested;$rejectAll=false;
  Queue::fake();$admin=$this->user('admin');$this->actingAs($admin)->postJson("/api/website-provisioning/{$run->id}/retry")->assertAccepted();Queue::assertPushed(\App\Jobs\ProcessWebsiteProvisioning::class,fn($job)=>$job->runId===$run->id);app(\App\Services\Hosting\WebsiteProvisioner::class)->process($run->fresh());$recoveryCalls=array_slice($requested,count($rejectedRequests));
  $this->assertSame('complete',$run->fresh()->state,(string)$run->fresh()->safe_error);$this->assertCount(1,$recoveryCalls);$this->assertNotContains($recoveryCalls[0],$rejectedRequests);$this->assertSame($recoveryCalls[0],$website->fresh()->cpanel_username);
 }
 public function test_username_rejection_reconciles_before_attempting_another_account():void
 {
  config(['hosting.provisioning_mode'=>'live','hosting.allow_live_provisioning'=>true]);$created=false;$createCalls=0;
  Http::fake(function($request)use(&$created,&$createCalls){if(str_contains($request->url(),'/listaccts'))return Http::response(['metadata'=>['result'=>1,'reason'=>'OK'],'data'=>['acct'=>$created?[['user'=>'ambiguoususr','domain'=>'ambiguous-username.test','ip'=>'1.2.3.4','plan'=>'Standard','suspended'=>0]]:[]]]);if(str_contains($request->url(),'/createacct')){$createCalls++;$created=true;return Http::response(['metadata'=>['result'=>0,'reason'=>'The username is unavailable.']]);}return Http::response([],404);});
  [$run,$website]=$this->singleStepLiveRun('ambiguous-username.test');$run->update(['secrets_encrypted'=>['cpanel_password'=>'private-password','cpanel_username_proposal'=>'ambiguoususr']]);app(\App\Services\Hosting\WebsiteProvisioner::class)->process($run);
  $this->assertSame(1,$createCalls);$this->assertSame('complete',$run->fresh()->state);$this->assertSame('ambiguoususr',$website->fresh()->cpanel_username);
 }
